...
public class Medications extends NavigationView {
    ...
    public Medications() {
        ...
        b.addClickListener(new ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
                try {
                    Bundle medBundle = getMedications();
                    ...
                    ApplySecurityLabelsFHIRForResourceReleaseResponse.Return r = port.applySecurityLabelsFHIRForResourceRelease(composeXMLStringFeed(medBundle), "1", hcs, session.getPurposeOfUse());
                    session.getPofPatient().reloadHCSMedications(r.getProcessedAtomFeed());
                    refreshTable();
                }
                catch(Exception ex) {
                    ex.printStackTrace();
