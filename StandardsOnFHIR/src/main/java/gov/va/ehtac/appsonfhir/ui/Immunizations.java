        ...
        b.addClickListener(new ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
                try {
                    ...
                    ApplySecurityLabelsFHIRForResourceReleaseResponse.Return r = port.applySecurityLabelsFHIRForResourceRelease(composeXMLStringFeed(bundle), "1", hcs, session.getPurposeOfUse());
                    session.getPofPatient().reloadHCSImmunizations(r.getProcessedAtomFeed());
                    refreshTable();
                }
                catch(Exception ex) {
                    ex.printStackTrace();
